...
class Swift_Transport_Esmtp_Auth_NTLMAuthenticatorTest extends \SwiftMailerTestCase
{
    ...
    public function testLMv1Generator()
    {
        ...
        $login = $this->_getAuthenticator();
        $lmv1Result = $this->_invokePrivateMethod('createLMPassword', $login, array(
            $password,
            $this->hex2bin($challenge)
        ));
    ...
    public function testLMv2Generator()
    {
        ...
        $login = $this->_getAuthenticator();
        $lmv2Result = $this->_invokePrivateMethod('createLMv2Password', $login, array(
            $password,
            $username,
            $domain,
            $this->hex2bin($challenge),
            $this->hex2bin('ffffff0011223344')
        ));
    ...
    public function testMessage3v1Generator()
    {
        ...
        $login = $this->_getAuthenticator();
        $message3 = $this->_invokePrivateMethod('createMessage3', $login, array(
            $domain,
            $username,
            $workstation,
            $this->hex2bin($lmResponse),
            $this->hex2bin($ntlmResponse)
        ));
    ...
    public function testGetDomainAndUsername()
    {
        ...
        $login = $this->_getAuthenticator();
        list ($domain, $user) = $this->_invokePrivateMethod('getDomainAndUsername', $login, array(
            $username
        ));
    ...
    public function testSuccessfulAuthentication()
    {
        ...
        $agent->shouldReceive('executeCommand')
            ->once()
            ->with('AUTH NTLM ' . base64_encode($this->_invokePrivateMethod('createMessage1', $ntlm)) . "\r\n", array(
            334
        ))
        ...
        $agent->shouldReceive('executeCommand')
            ->once()
            ->with(base64_encode($this->_invokePrivateMethod('createMessage3', $ntlm, array(
            $domain,
            $username,
