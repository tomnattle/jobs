...
class Swift_CharacterReader_GenericFixedWidthReaderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testValidationValueIsBasedOnOctetCount()
    {
        ...
        $this->assertSame(1, $reader->validateByteSequence(array(
            0x01,
            0x02,
            0x03
        ), 3)); // 3 octets
        
        $this->assertSame(2, $reader->validateByteSequence(array(
            0x01,
            0x0A
        ), 2)); // 2 octets
        
        $this->assertSame(3, $reader->validateByteSequence(array(
            0xFE
        ), 1)); // 1 octet
        
        $this->assertSame(0, $reader->validateByteSequence(array(
            0xFE,
            0x03,
            0x67,
            0x9A
