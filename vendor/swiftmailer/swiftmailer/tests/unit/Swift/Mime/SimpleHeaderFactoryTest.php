...
class Swift_Mime_SimpleHeaderFactoryTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $header = $this->_factory->createMailboxHeader('X-Foo', array(
            'foo@bar' => 'FooBar'
        ));
    ...
    {
        $header = $this->_factory->createParameterizedHeader('X-Foo', 'bar', array(
            'zip' => 'button'
        ));
        $this->assertEquals(array(
            'zip' => 'button'
    ...
    {
        return new Swift_Mime_SimpleHeaderFactory($encoder ? $encoder : $this->_createHeaderEncoder(), $paramEncoder ? $paramEncoder : $this->_createParamEncoder(), new Swift_Mime_Grammar());
    }
