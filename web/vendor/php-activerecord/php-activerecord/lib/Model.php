...
class Model
{
    ...
    /**
     * Retrieves an attribute's value or a relationship object based on the name passed.
     * If the attribute
     ...
     *
     * @param
     *            boolean Set to true to return the first value in the pk array only
     ...
     *            An array containing delegate data
     ...
     *            Name of method that was invoked on model for exception message
     */
     ...
     * @param boolean $guard_attributes
     *            Flag of whether or not protected/non-accessible attributes should be guarded
     */
    ...
    public function __call($method, $args)
    {
        ...
        if (preg_match('/(build|create)_/', $method)) {
            ...
            $association_name = str_replace(array(
                'build_',
                'create_'
            ), '', $method);
	...
	{
        return call_user_func_array('static::find', array_merge(array(
            'all'
        ), func_get_args()));
    }
...
elseif (1 === count($args) && 1 == $num_args)
     ...
     * @see Serialization
     * @param array $options
     *            An array containing options for json serialization (see {@link Serialization} for valid options)
     ...
     * @see Serialization
     * @param array $options
     *            An array containing options for xml serialization (see {@link Serialization} for valid options)
     ...
     * @see Serialization
     * @param array $options
     *            An array containing options for csv serialization (see {@link Serialization} for valid options)
     ...
     *
     * @param string $type
     *            Either Xml, Json, Csv or Array
     * @param array $options
     *            Options array for the serializer
