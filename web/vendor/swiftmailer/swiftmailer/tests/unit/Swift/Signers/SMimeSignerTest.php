...
class Swift_Signers_SMimeSignerTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $message = Swift_SignedMessage::newInstance('Wonderful Subject')->setFrom(array(
...
OEL;
    ...
    public function testEncryptedMessage()
    {
        ...
        if (! openssl_pkcs7_decrypt($messageStream->getPath(), $decryptedMessageStream->getPath(), 'file://' . $this->samplesDir . 'smime/encrypt.crt', array(
    ...
    public function testEncryptedMessageWithMultipleCerts()
    {
        ...
        if (! openssl_pkcs7_decrypt($messageStream->getPath(), $decryptedMessageStream->getPath(), 'file://' . $this->samplesDir . 'smime/encrypt2.crt', array(
...
OEL;
        ...
        if (! openssl_pkcs7_decrypt($messageStreamClean->getPath(), $decryptedMessageStream->getPath(), 'file://' . $this->samplesDir . 'smime/encrypt.crt', array(
            'file://' . $this->samplesDir . 'smime/encrypt.key',
            'swift'
        ))) {
            $this->fail(sprintf('Decrypt of the message failed. Internal error "%s".', openssl_error_string()));
    ...
    protected function assertValidVerify($expected, Swift_ByteStream_TemporaryFileByteStream $messageStream)
    {
        ...
        $verify = openssl_pkcs7_verify($messageStream->getPath(), null, $opensslOutput->getPath(), array(
