...
X-My-Header:Value2\r\n";
    ...
    {
        $r = Request::init()->withStrictSsl();
        
        $this->assertTrue($r->strict_ssl);
        
        $r = Request::init()->withoutStrictSsl();
    ...
    function testIni()
    {
        ...
        // Create the template
        $template = Request::init()->method(Http::POST)
            ->withStrictSsl()
    ...
    function testAccept()
    {
        $r = Request::get('http://example.com/')->expectsType(Mime::JSON);
    ...
    function testCustomAccept()
    {
        ...
        $r = Request::get('http://example.com/')->addHeader('Accept', $accept);
    ...
    {
        $r = Request::get('http://example.com/')->withUserAgent('ACME/1.2.3');
        ...
        $r = Request::get('http://example.com/')->withUserAgent('');
    ...
    function testAuthSetup()
    {
        ...
        $r = Request::get('http://example.com/')->authenticateWith($username, $password);
    ...
    function testDigestAuthSetup()
    {
        ...
        $r = Request::get('http://example.com/')->authenticateWithDigest($username, $password);
...
Content-Type: text/plain; charset=utf-8\r\n", $req);
    ...
    function testWhenError()
    {
        ...
        try {
            Request::get('malformed:url')->whenError(function ($error) use (&$caught) {
                $caught = true;
            })
                ->timeoutIn(0.1)
                ->send();
    ...
    function testBeforeSend()
    {
        ...
        try {
            Request::get('malformed://url')->beforeSend(function ($request) use (&$invoked, $self) {
