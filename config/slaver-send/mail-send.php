...
$_config = [
    ...
    'dev' => [
        ...
        'downloader_uri' => function ($accessToken, $projectId, $fileName) {
            return 'http://upload2.dev.*.com/?access-token=server-side&projectId=' . $projectId . '&filename=' . $fileName;
        },
        'import_api' => function ($projectId) {
            return 'http://contact2.*.dev/rest/v2/' . $projectId . '/users';
        }
    ...
    'beta' => [
        ...
        'json_path' => function ($projectId, $taskId) {
         ...
         /**
        'upload_uri' => function () {
            return "http://*.*.62.202:8667";
        },
        'downloader_uri' => function ($accessToken, $projectId, $fileName) {
            return 'http://*.*.62.202:8667/?access-token=server-side&projectId=' . $projectId . '&filename=' . $fileName;
        },
        'import_api' => function ($projectId) {
